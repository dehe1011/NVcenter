...
class Environment(Pulse):
    ...
    def step(self, action):
        ...
        if self.instant_pulses:
            self.action1.append(2*np.pi*action[1])
        else: 
            self.action1.append(action[1])
        self.action2.append(2*np.pi*action[2])
        
        self.observation = [*self.action0+[0], *self.action1, *self.action2]
        self.fidelity = self.get_values(self.observation, 'fidelity')[0]
    ...
    def get_values(self, pulse_seq, observable, t_list='final'):
        ...
        for i in tqdm(range(self.num_bath_configs), desc=message, disable=disable_tqdm):
            ...
            for i, idx_gCCE2 in enumerate(self.idx_gCCE2):
                j, k = idx_gCCE2
                values_gCCE *= np.divide( np.multiply(values_full_gCCE2[i], values_full_gCCE0[0]), np.multiply(values_full_gCCE1[j], values_full_gCCE1[k]) )
    ...
    def get_new_register_states(self, pulse_seq, t_list='final'):
        ...
        for i in tqdm(range(self.num_bath_configs), desc=message, disable=disable_tqdm):
            ...
            # gCCE0 result
            if self.env_approx_level == 'gCCE0':
                states_gCCE_baths += np.array([q.Qobj(state, dims=self.dims) for state in states_gCCE])
                continue
            ...
            for i, idx_gCCE2 in enumerate(self.idx_gCCE2):
                j, k = idx_gCCE2
                states_gCCE *= np.divide( np.multiply(states_full_gCCE2[i], states_full_gCCE0[0]), np.multiply(states_full_gCCE1[j], states_full_gCCE1[k]) )
